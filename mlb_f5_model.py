...
def fetch_pinnacle_odds(target_date):
    """Fetch 1st-5-innings H2H + totals for MLB games. F5 odds use the
    per-event Odds API endpoint (slate endpoint doesn't expose F5 markets).

    Pulls Pinnacle (eu region) and FanDuel (us region) together for each game.
    - Totals: take Pinnacle's, fall back to FanDuel.
    - Moneyline: take Pinnacle's when present (Pinnacle sometimes doesn't
      offer F5 h2h in eu region), fall back to FanDuel.

    Returns a dict keyed by AWAY@HOME tab name. Cost: ~1 credit per game.
    """
    try:
    ...
    # First fetch the list of upcoming events (1 credit) with retry.
    import time as _time
    events = None
    ...
    n_ml_missing = 0
    diag_printed = False
    print(f"  F5 odds: pulling per-event markets for {len(events)} games "
          f"(~{len(events)} credits)...")

    def _parse_book(book, away_team, home_team):
        """Pull (away_ml, home_ml, total, over, under) from one bookmaker."""
        a_ml = h_ml = None
        tot = ov = un = None
        for mkt in book.get("markets", []):
            if mkt["key"] == "h2h_1st_5_innings":
                for o in mkt["outcomes"]:
                    if o["name"] == away_team: a_ml = int(o["price"])
                    elif o["name"] == home_team: h_ml = int(o["price"])
            elif mkt["key"] == "totals_1st_5_innings":
                for o in mkt["outcomes"]:
                    if o["name"] == "Over":
                        tot = float(o.get("point", 0)); ov = int(o["price"])
                    elif o["name"] == "Under":
                        un = int(o["price"])
        return a_ml, h_ml, tot, ov, un
    ...
    for ev in events:
        ...
        if r2.status_code != 200:
            continue
        payload = r2.json()
        books = {b["key"]: b for b in payload.get("bookmakers", [])}

        # Diagnostics on the first event
        if not diag_printed:
            diag_printed = True
            print(f"    [diag] {tab}: bookmakers returned = {list(books.keys())}")
            for bk_key, bk in books.items():
                mkts = [m["key"] for m in bk.get("markets", [])]
                print(f"    [diag]   {bk_key} markets = {mkts}")

        pinny = books.get("pinnacle")
        fd = books.get("fanduel")

        p_a_ml = p_h_ml = p_tot = p_ov = p_un = None
        if pinny:
            p_a_ml, p_h_ml, p_tot, p_ov, p_un = _parse_book(
                pinny, ev["away_team"], ev["home_team"])
        f_a_ml = f_h_ml = f_tot = f_ov = f_un = None
        if fd:
            f_a_ml, f_h_ml, f_tot, f_ov, f_un = _parse_book(
                fd, ev["away_team"], ev["home_team"])

        # Totals: Pinnacle first, FanDuel fallback
        total = p_tot if p_tot is not None else f_tot
        over = p_ov if p_ov is not None else f_ov
        under = p_un if p_un is not None else f_un
        total_src = "pinnacle" if p_tot is not None else ("fanduel" if f_tot is not None else None)

        # ML: Pinnacle first, FanDuel fallback
        if p_a_ml is not None and p_h_ml is not None:
            away_ml, home_ml, ml_src = p_a_ml, p_h_ml, "pinnacle"
            n_ml_pinnacle += 1
        elif f_a_ml is not None and f_h_ml is not None:
            away_ml, home_ml, ml_src = f_a_ml, f_h_ml, "fanduel"
            n_ml_fanduel += 1
        else:
            away_ml, home_ml, ml_src = None, None, None
            n_ml_missing += 1

        if away_ml is None and total is None:
            continue
        out[tab] = {
            "away_ml": away_ml, "home_ml": home_ml,
            "total": total, "over_juice": over, "under_juice": under,
            "ml_source": ml_src, "total_source": total_src,
            "last_update": (pinny or fd or {}).get("last_update", ""),
            "event_id": ev["id"],
        }
        n_priced += 1

    print(f"  Got F5 odds for {n_priced} games "
          f"(ML: {n_ml_pinnacle} Pinnacle, {n_ml_fanduel} FanDuel, "
          f"{n_ml_missing} missing).")
    return out
