...
def sample_results():
    ...
    # Attach realistic multi-book odds to each priced game.
    import random as _rnd
    _rnd.seed(7)
    BOOKS = ["DraftKings","FanDuel","BetMGM","Caesars","Fanatics","Fliff"]
    for r in G:
        if r.get("status") != "ok":
            continue
        def _spread(med, n=3):
            vals = sorted({med + _rnd.choice([-8,-6,-4,0,4,6,8]) for _ in range(n+2)})
            return vals
        a_med = r["away_book"]; h_med = r["home_book"]
        ov = r.get("total_over_odds") or -110; un = r.get("total_under_odds") or -110
        def best(med):
            cand = _spread(med); return max(cand)
        r["_odds"] = {
            "away_ml_best": best(a_med), "away_ml_book": _rnd.choice(BOOKS),
            "home_ml_best": best(h_med), "home_ml_book": _rnd.choice(BOOKS),
            "over_juice": ov, "over_best": best(ov), "over_book": _rnd.choice(BOOKS),
            "under_juice": un, "under_best": best(un), "under_book": _rnd.choice(BOOKS),
            "n_books": 6,
        }
